  ...
  const handleSave = async () => {
    try {
      ...
      if (id) {
        ...
        setToast({ open: true, message: 'Service updated successfully!', severity: 'success' });
      } else {
        await createTransaction(data);
        setToast({ open: true, message: 'Service saved successfully!', severity: 'success' });
        setTimeout(() => navigate('/services'), 1500);
      }
    } catch (err) {
      console.error(err);
      setToast({ open: true, message: 'Error saving service', severity: 'error' });
    }
  };
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <Button variant="contained" color="info" startIcon={<SaveIcon />} onClick={handleSave}>Save Service</Button>
        </Box>
      </Box>
      ...
      <Paper sx={{ p: 4, borderRadius: 2 }}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth label="Service Number" value={formData.transactionNumber || ''} onChange={handleChange('transactionNumber')} size="small" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth label="Client Name" value={formData.partAccountName || ''} onChange={handleChange('partAccountName')} size="small" />
          </Grid>
        </Grid>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth label="Items" value={formData.item || ''} onChange={handleChange('item')} size="small" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
          ...
          <Grid item xs={12} md={3}>
            <TextField fullWidth label="Problem Description" value={formData.description || ''} onChange={handleChange('description')} size="small" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth label="Charges (₹)" value={formData.rate || ''} onChange={handleChange('rate')} type="number" size="small" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <TextField fullWidth label="Total Amount (₹)" disabled value={formData.rate || ''} size="small" />
          </Grid>
        </Grid>
